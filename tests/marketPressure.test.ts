...
test("market pressure emits sharp-led and lag signals only with evidence", () => {
  ...
  assert.ok(labels.includes("sharp-led move"));
  assert.ok(labels.includes("mainstream lagging"));
  assert.ok(labels.includes("isolated stale quote"));
});
...
test("market pressure avoids sharp-led label when sequence evidence is missing", () => {
  ...
  const signals = detectMarketPressure(points);
  assert.equal(signals.some((signal) => signal.label === "sharp-led move"), false);
});
